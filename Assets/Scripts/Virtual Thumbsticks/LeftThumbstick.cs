...
public class LeftThumbstick : MonoBehaviour {
	...
	void OnGUI()
	{
		if (ConstManager.DebugMode)
		{
			...
			{
				GUI.Label(new Rect(300, Screen.height - 20, 100, 20), new GUIContent("Not Updating Left Thumbstick"));
			}
			else
			{
				GUI.Label(new Rect(300, Screen.height - 20, 100, 20), new GUIContent("Updating Left Thumbstick"));

				Vector2 guiSpace = ConvertScreenSpaceToGuiSpace(VirtualJoysticks.LeftThumbstickCenter.Value);
